...
int get_module_base(pid_t pid, const char* module_name, struct addr *p)
{
        FILE *fp;
        char *pch;
        char filename[32];
        ...
        if (fp != NULL) {
                while (fgets(line, sizeof(line), fp)) {
                        if (strstr(line, module_name)) {
                                pch = strtok( line, "-" );
                                p->addr_begin = strtoul( pch, NULL, 16 ); //起始地址
                                pch = strtok(NULL,"-");
                                p->addr_end = strtoul(pch,NULL,16); //结束地址
                                if (p->addr_begin == 0x8000)
                                        p->addr_begin = 0;
...
int hook(struct hook_t *h, int pid, char *libname, char *funcname, void *hook_arm, void *hook_thumb)
{
	...
	sprintf(buf,"%sso",libname);
	i = get_module_base(pid,buf,&soaddr);
	if(i != 1){
		log("get_module_base error\n");
	}
	
	int32_t page_size = getpagesize();
        int32_t protect_size = soaddr.addr_end - soaddr.addr_begin;
        uint32_t entry_page_start = ((int)soaddr.addr_begin)&(~(page_size-1)); //对齐
        int _size = ((int)protect_size)&(~(page_size-1));             //对齐
        if((mprotect((void*)entry_page_start , _size, PROT_READ|PROT_WRITE|PROT_EXEC)) != 0){
                log("mprotect error \n");
        }
